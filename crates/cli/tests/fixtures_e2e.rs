...
fn cc_fixture_token_totals_match_expected() {
    ...
    let expected: Value = serde_json::from_str(
        &fs::read_to_string(fixture_dir().join("cc/sample.expected.json")).expect("read expected"),
    )
    .expect("expected JSON parses");

    assert_eq!(token_counts_to_value(counts), expected);
}
...
fn cc_fixture_dryrun_snapshot_locks() {
    ...
    assert_eq!(
        output,
        fs::read_to_string(fixture_dir().join("cc/sample.expected.dryrun.txt"))
            .expect("read expected dryrun")
    );
}
...
fn codex_fixture_token_totals_match_expected() {
    ...
    let counts =
        bloclawd_cli::aggregate::aggregate(&[], &events, WindowKind::FiveHour).expect("aggregate");
    let expected: Value = serde_json::from_str(
        &fs::read_to_string(fixture_dir().join("codex/sample.expected.json"))
...
fn codex_fixture_dryrun_snapshot_locks() {
    ...
    assert_eq!(
        output,
        fs::read_to_string(fixture_dir().join("codex/sample.expected.dryrun.txt"))
            .expect("read expected dryrun")
    );
}
