...
//!
//! Wire contract, locked by spec/pow-v1.md:
//!   200 {
...
//!
//! NO console_log of challenge_id, sig, random bytes, secret, or timing.
...
/// Challenge expiry in seconds. Echoed in the response
...
pub async fn handle_challenge(req: Request, ctx: RouteContext<()>) -> Result<Response> {
    // Step 1: RL_CHALLENGE per IP. Single concern.
    ...
    // Step 4: WORKER_SECRET from per-env secret. Drops on return.
    ...
    // Same primitive used by the e2e test: wire identical.
    let (cid, sig) = issue_challenge(secret.as_bytes(), now_ms, random_24);

    // Step 6: encode for wire - base64url no padding.
    let body = json!({
    ...
    // NO console_log here. The values above were generated for this
