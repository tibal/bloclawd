...
APP_NAME="bloclawd"
APP_VERSION="0.1.2"
if [ -n "${BLOCLAWD_DOWNLOAD_URL:-}" ]; then
    ARTIFACT_DOWNLOAD_URLS="$BLOCLAWD_DOWNLOAD_URL"
elif [ -n "${INSTALLER_DOWNLOAD_URL:-}" ]; then
    ARTIFACT_DOWNLOAD_URLS="$INSTALLER_DOWNLOAD_URL"
elif [ -n "${BLOCLAWD_INSTALLER_GHE_BASE_URL:-}" ]; then
    INSTALLER_BASE_URL="$BLOCLAWD_INSTALLER_GHE_BASE_URL"
    ARTIFACT_DOWNLOAD_URLS="${INSTALLER_BASE_URL}/bloclawd/bloclawd/releases/download/v0.1.2"
elif [ -n "${BLOCLAWD_INSTALLER_GITHUB_BASE_URL:-}" ]; then
    INSTALLER_BASE_URL="$BLOCLAWD_INSTALLER_GITHUB_BASE_URL"
    ARTIFACT_DOWNLOAD_URLS="${INSTALLER_BASE_URL}/bloclawd/bloclawd/releases/download/v0.1.2"
else
    ARTIFACT_DOWNLOAD_URLS="https://github.com/bloclawd/bloclawd/releases/download/v0.1.2"
fi
if [ -n "${BLOCLAWD_PRINT_VERBOSE:-}" ]; then
    PRINT_VERBOSE="$BLOCLAWD_PRINT_VERBOSE"
else
    PRINT_VERBOSE=${INSTALLER_PRINT_VERBOSE:-0}
fi
if [ -n "${BLOCLAWD_PRINT_QUIET:-}" ]; then
    PRINT_QUIET="$BLOCLAWD_PRINT_QUIET"
else
    PRINT_QUIET=${INSTALLER_PRINT_QUIET:-0}
fi
if [ -n "${BLOCLAWD_NO_MODIFY_PATH:-}" ]; then
    NO_MODIFY_PATH="$BLOCLAWD_NO_MODIFY_PATH"
else
    NO_MODIFY_PATH=${INSTALLER_NO_MODIFY_PATH:-0}
fi
if [ "${BLOCLAWD_DISABLE_UPDATE:-0}" = "1" ]; then
    INSTALL_UPDATER=0
else
    INSTALL_UPDATER=1
fi
UNMANAGED_INSTALL="${BLOCLAWD_UNMANAGED_INSTALL:-}"
if [ -n "${UNMANAGED_INSTALL}" ]; then
    NO_MODIFY_PATH=1
    INSTALL_UPDATER=0
fi
AUTH_TOKEN="${BLOCLAWD_GITHUB_TOKEN:-}"

read -r RECEIPT <<EORECEIPT
{"binaries":["CARGO_DIST_BINS"],"binary_aliases":{},"cdylibs":["CARGO_DIST_DYLIBS"],"cstaticlibs":["CARGO_DIST_STATICLIBS"],"install_layout":"unspecified","install_prefix":"AXO_INSTALL_PREFIX","modify_path":true,"provider":{"source":"cargo-dist","version":"0.31.0"},"source":{"app_name":"bloclawd","name":"bloclawd","owner":"bloclawd","release_type":"github"},"version":"0.1.2"}
EORECEIPT
...
if is_windows_posix && [ -n "${LOCALAPPDATA:-}" ]; then
    RECEIPT_HOME="$LOCALAPPDATA/bloclawd"
else
    RECEIPT_HOME="${XDG_CONFIG_HOME:-$INFERRED_HOME/.config}/bloclawd"
fi

usage() {
    # print help (this cat/EOF stuff is a "heredoc" string)
    cat <<EOF
bloclawd-installer.sh

The installer for bloclawd 0.1.2

This script detects what platform you're on and fetches an appropriate archive from
https://github.com/bloclawd/bloclawd/releases/download/v0.1.2
...
download_binary_and_run_installer() {
    ...
    for arg in "$@"; do
        case "$arg" in
            ...
            --verbose)
                ...
                ;;
            --no-modify-path)
                say "--no-modify-path has been deprecated; please set BLOCLAWD_NO_MODIFY_PATH=1 in the environment"
                NO_MODIFY_PATH=1
                ;;
            *)
                OPTIND=1
    ...
    case "$_artifact_name" in 
        "bloclawd-aarch64-apple-darwin.tar.xz")
            _arch="aarch64-apple-darwin"
            _zip_ext=".tar.xz"
            _checksum_style="sha256"
            _checksum_value="f3698463e2969c7142481979fa63f0c83bcc101915f760336c20eca65f4d9767"
            _bins="bloclawd"
            _bins_js_array='"bloclawd"'
            _libs=""
            _libs_js_array=""
            _staticlibs=""
            ...
            _updater_bin=""
            ;;
        "bloclawd-x86_64-apple-darwin.tar.xz")
            _arch="x86_64-apple-darwin"
            _zip_ext=".tar.xz"
            _checksum_style="sha256"
            _checksum_value="718640535d30ae2873c200bd6665b447f8034601510fa5917290c68fa750d449"
            _bins="bloclawd"
            _bins_js_array='"bloclawd"'
            _libs=""
            _libs_js_array=""
            ...
            _updater_bin=""
            ;;
        "bloclawd-x86_64-unknown-linux-musl.tar.xz")
            _arch="x86_64-unknown-linux-musl-static"
            _zip_ext=".tar.xz"
            _checksum_style="sha256"
            _checksum_value="9d66a0be3b93cc39093101c9866e9db214da714dc7ab5cf43056b2d68fc3cfb6"
            _bins="bloclawd"
            _bins_js_array='"bloclawd"'
            _libs=""
            _libs_js_array=""
...
select_archive_for_arch() {
    ...
    case "$_true_arch" in 
        "aarch64-apple-darwin")
            _archive="bloclawd-aarch64-apple-darwin.tar.xz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            _archive="bloclawd-x86_64-apple-darwin.tar.xz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "x86_64-apple-darwin")
            ...
            ;;
        "x86_64-unknown-linux-gnu")
            _archive="bloclawd-x86_64-unknown-linux-musl.tar.xz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
...
install() {
    ...
    if [ -n "${BLOCLAWD_INSTALL_DIR:-}" ]; then
        _force_install_dir="$BLOCLAWD_INSTALL_DIR"
