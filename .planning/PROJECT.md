...
- **Aggregation outlier policy**: Per-cohort 2σ trim on the unified per-submission cost. cohort = `(tier, harness, region)`. The unified cost is computed via per-(cohort, limit_type) ridge regression toward published per-token priors (see AGGR-15). Reason: 2σ on a unified cost is interpretable across mixed-model usage and the ridge prior keeps cold-start cohorts meaningful. Trim-rate is surfaced as a metric.
- **Anonymity is combinatorial**: (a) k-anonymity (k=5) suppression of public R2 cells; (b) token counts binned to log-spaced buckets before public emission; (c) `tz_offset` dropped from public R2 output; (d) submission_group_id / event_id / nonce never written to public R2 files; (e) per-event timing (solve duration, sub-minute submit time) never persisted. Reason: "we don't store IP" alone is a fingerprintable schema (Sweeney-style re-identification).
...
| Per-cohort 2σ trim on unified per-submission cost (cohort = (tier, harness, region)) before percentile aggregation; per-(cohort, limit_type) ridge weight fit with Bayesian prior | 2σ on a unified per-submission cost is interpretable across mixed-model usage; ridge regression with prior keeps low-volume cohorts meaningful (D-80, D-82, AGGR-15) | — Pending |
...
*2026-05-02 — Phase 4 Wave 0: cohort key changed to (tier, harness, region); outlier policy changed to 2σ on unified cost; submission_group_id added to anonymity-strip list.*
