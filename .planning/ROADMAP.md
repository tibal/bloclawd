...
**Depends on**: Phase 3 fully merged (plans 03-00..03-07 — CLI fixture path + submission_group_id wire field + staging events table populated by a cargo-test --features staging-smoke run). Phase 4 wave ≥ 2 plans assert this via `<preflight>` blocks; do not start them while Phase 3 is open.
**Requirements**: AGGR-01..18, WEB-01..17
**Success Criteria** (what must be TRUE):
  1. The Worker `scheduled` handler runs at a config-driven cadence (default daily for v1) via the same Worker as ingest, GROUPs `events` by `submission_group_id` within each closed bucket, fits per-(cohort, limit_type) ridge weights toward published per-token priors, computes a unified per-submission cost, applies 2σ trim on the unified per-submission cost (cohort = `(tier, harness, region)`) before computing p10/p25/p50/p75/p90, suppresses any (cohort, limit_type) cell with n_distinct_submission_groups < 5, encodes percentiles via a windowed L-estimator with powers-of-2 log-bin fallback, drops `tz_offset`, and never writes `submission_group_id`/`event_id`/`nonce` to any R2 file; trim-rate is surfaced as a metric (>5% warns, >10% escalates).
...
**Plans**: 19 plans (sequenced into 7 waves; Wave 0 docs-amendments blocks all code; Wave 1 shared types; Wave 2 SQL+Worker+CLI cross-phase amendments; Wave 3 cron module + frontend scaffold parallel; Wave 4 frontend tokens/routes/lib parallel with cron amendments; Wave 5 dashboard assembly + methodology/data routes; Wave 6 tests+CI gates; Wave 7 staging proof checkpoint)
Plans:
- [ ] **Wave 0** *(docs amendments — blocks all code)*
  - [x] 04-00-docs-amendments-PLAN.md — REQUIREMENTS.md AGGR-01/02/04/05/06 amend + AGGR-15..18 add (D-87); ROADMAP.md Phase 4 SC#1+SC#4 amend (D-88); PROJECT.md Key Decisions table; CLAUDE.md Outlier policy line (D-80); spec/event-schema.md §1 limit_type field (D-84)
- [ ] **Wave 1** *(shared crate amendments; blocked on Wave 0)*
  - [x] 04-01-shared-types-PLAN.md — crates/event-schema: LimitType + TokenType + Window enums; SubmittedEvent.limit_type field; log_bins.rs (D-96); model_prices.rs (D-102); ts-rs regen + barrel; pow.yml log-boundary grep extension
- [ ] **Wave 2** *(cross-phase amendments; blocked on Wave 1)*
  - [x] 04-02-sql-migrations-PLAN.md — apps/worker/sql/0003_cron_state.sql + 0004_add_limit_type.sql + README operator notes (R2 buckets, Workers Paid plan)
  - [x] 04-03-worker-event-amendment-PLAN.md — apps/worker/src/event.rs limit_type deserialize/insert; INSERT_EVENT_SQL extends to $8::text; enum_invalid_field includes "limit_type"
  - [x] 04-04-cli-fixture-regen-PLAN.md — crates/cli sets LimitType from --5h/--week; fixture regen (CLI-17)
- [ ] **Wave 3** *(cron module + frontend scaffold; blocked on Wave 2; cron and frontend halves run parallel)*
  - [x] 04-05-cron-state-PLAN.md — cron module skeleton + cron/state.rs work-queue helpers (AGGR-17/18); #[event(scheduled)] entry stub
  - [x] 04-06-cron-aggregate-ridge-PLAN.md — cron/ridge.rs (hand-rolled Gauss-Jordan) + cron/aggregate.rs (compute_cells: group by submission_group_id, ridge fit, 2σ trim, k-anon, per-model derivation) + golden fixtures (AGGR-02/03/15/16)
  - [x] 04-07-cron-percentile-PLAN.md — cron/percentile.rs (windowed L-estimator + log-bin fallback per AGGR-04/06; threshold n_trimmed≥21)
  - [x] 04-08-cron-r2-emit-PLAN.md — cron/r2_emit.rs (path conventions AGGR-09; HttpMetadata cache headers AGGR-13; manifest-last via orchestrator) + wrangler.toml r2_buckets per env
  - [x] 04-09-cron-health-PLAN.md — cron/health.rs (StatusJson + classify_health + count_lifetime_events + fuzzy-rounded contributors per AGGR-14 + Pitfall 7)
  - [x] 04-10-cron-orchestrator-PLAN.md — cron/tick.rs full impl (sweep → eager_fill → claim → process → write_status → rewrite_manifest LAST per AGGR-01) + wrangler.toml [triggers] crons per env
  - [x] 04-11-frontend-scaffold-PLAN.md — apps/frontend Vite 6 + React 19 + TanStack Router/Query + Tailwind v4 + shadcn 4.6 init scaffold (WEB-01)
- [ ] **Wave 4** *(frontend content; blocked on Wave 3 frontend scaffold)*
  - [x] 04-12-frontend-tokens-routes-PLAN.md — tokens.css OKLCH light+dark (WEB-12); 5 route shells (WEB-02/16/17/13); RouteShell + EmptyState
  - [x] 04-13-frontend-r2-canonical-PLAN.md — lib/r2.ts (TanStack Query + PromisePool cap 8 per WEB-06) + lib/tier-picker.ts (WEB-07) + lib/canonical.ts (TS hand-port of JCS for WEB-15) + cross-language fixture gate
  - [x] 04-14-frontend-chart-components-PLAN.md — Chart.tsx uPlot wrapper (D-110, WEB-03/10/11) + Filters/BandToggle/TierToggle (URL-synced WEB-04) + Chrome (WEB-08/09) + DataTable (WEB-11) + CodeBlock + FieldAnnotation
- [ ] **Wave 5** *(dashboard assembly + content pages; blocked on Wave 4)*
  - [x] 04-15-frontend-dashboard-assembly-PLAN.md — /dashboard route assembly + useChartData hook (WEB-03/04/05/07 + D-108/D-109)
  - [x] 04-16-frontend-methodology-data-PLAN.md — /methodology page content (WEB-14) + /data two-pane (WEB-15) + /methodology/changelog confirm (WEB-17)
- [ ] **Wave 6** *(tests + CI gates; blocked on Wave 5)*
  - [x] 04-17-tests-CI-gates-PLAN.md — pow.yml strip-at-cron grep (AGGR-07/08) + frontend-build-test job + R2 schema drift gate + cron::tests::integration + SPA dashboard-e2e test
- [ ] **Wave 7** *(staging proof; blocked on Wave 6)*
  - [ ] 04-18-staging-proof-PLAN.md — cron_e2e_staging.rs end-to-end (#[ignore] + staging-smoke); .env.staging + .env.production; checkpoint:human-verify on deployed staging stack
**UI hint**: yes
...
*2026-05-02 — Phase 4 Wave 0: SC#1 + SC#4 amended per D-88 (daily-cadence default, 2σ unified-cost trim, cohort = (tier, harness, region), per-limit-type axis); Phase 4 Requirements line extended to AGGR-01..18.*
