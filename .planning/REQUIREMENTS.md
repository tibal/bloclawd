...
- [ ] **AGGR-01**: Cron runs at config-driven cadence (default daily for v1; narrows as data volume justifies, documented in `/methodology`) via Worker `scheduled` handler in same Worker as ingest. q15/h1/d1 file paths preserved across cadence changes so the public R2 contract is stable.
- [ ] **AGGR-02**: Per-cohort 2σ trim on the unified per-submission cost (where unified cost = `Σ_M Σ_T Σ_W (tokens_{M,T,W} × w_{M,T,W})` with weights fit per (cohort, limit_type) — see AGGR-15) before percentile aggregation. cohort = `(tier, harness, region)`.
- [ ] **AGGR-03**: Trim rate per cohort surfaced as a metric; >5% triggers warning, >10% triggers escalation
- [ ] **AGGR-04**: Per-cohort × per-limit-type unified-cost percentiles (p10..p90) computed from the trimmed sample. Encoding = windowed L-estimator (centered window of 5 trimmed samples → arithmetic mean) when all 5 percentile windows fit (`n_trimmed ≥ 21`, derived from floor-rule rank + 5-window centered fit; pinned in 04-07); else powers-of-2 log-bin index with edges defined in `crates/event-schema/src/log_bins.rs` and inlined into each top-level R2 file.
- [ ] **AGGR-05**: k-anonymity enforced at materialization: cells (cohort × limit_type) with `n_distinct_submission_groups < 5` carry `insufficient_data: true` and emit no percentile / model data. n is the count of distinct `submission_group_id`s contributing to the cell, NOT raw event count.
- [ ] **AGGR-06**: Token counts NEVER published as raw integers. The windowed L-estimator emits a smoothed mean of ≥5 trimmed neighbors; the powers-of-2 bin fallback emits a bin index. Both encodings preserve the no-individual-value-recoverable property.
...
- [ ] **AGGR-15**: Per-(cohort, limit_type) weight fit. 8 weights per model (per token_type × window). Ridge regression with Bayesian prior toward `crates/event-schema/src/model_prices.rs` (researcher-populated from public Anthropic + OpenAI per-token pricing): `argmin_w ||cost - X w||² + λ||w − prior||²` with λ ∝ 1/n. Stratified fallback when n_trimmed < N_fit: per-cohort → (tier, harness) global → tier global → prior verbatim. Each model entry carries `weight_source: 'cohort' | 'tier+harness' | 'tier' | 'prior'` for transparency.
- [ ] **AGGR-16**: Per-model `tokens_to_limit_if_only` derivative emitted only when `n_with_model >= 5` in the cohort. Below threshold, model entry carries `tokens_to_limit_if_only: null` (SPA renders 'NA').
- [ ] **AGGR-17**: Cron uses a per-(tier, bucket_ts) work queue (`cron_state` table per D-85) with state machine `not_processed → processing → processed`. Atomic claim via `BEGIN; SELECT ... FOR UPDATE SKIP LOCKED; UPDATE ... SET state='processing'; COMMIT;` over Hyperdrive + tokio-postgres. On materialization failure the row reverts to `not_processed` (with `last_error` populated). Stale-claim sweeper at top of each tick resets `processing` rows older than 5 × cron_interval back to `not_processed`.
- [ ] **AGGR-18**: Eager INSERT-where-not-exists at cron tick start fills `cron_state` with all (tier, bucket_ts) work items between the last-processed bucket and `now() - lateness_window` (default 30 min for sub-daily cadence, 0 for daily cadence).
...
| AGGR-15 | Phase 4 | Pending |
| AGGR-16 | Phase 4 | Pending |
| AGGR-17 | Phase 4 | Pending |
| AGGR-18 | Phase 4 | Pending |
...
- Phase 4 (Aggregation + Dashboard): 35 requirements (AGGR-01..18, WEB-01..17)
...
*2026-05-02 — Phase 4 Wave 0: AGGR-01/02/04/05/06 amended for 2σ unified-cost trim + (tier, harness, region) cohort + per-limit-type axis + windowed L-estimator + log-bin fallback. AGGR-15..18 added (ridge weight fit, per-model k-anon gate, cron_state work queue, eager fill). Phase 4 count 31 → 35; total 82 → 86.*
