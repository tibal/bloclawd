...
**`limit_type`** (`"5h" | "weekly"`, REQUIRED). Top-level transport field — NOT inside payload (mirrors submission_group_id placement per D-52). Source: CLI flag (`--5h` | `--week`). Used by Phase 4 cron to split each cohort into per-limit-type cells.
...
---
*2026-05-02 — Phase 4 D-84: limit_type added as wire envelope field.*
