...
- **Database:** PlanetScale Postgres via Hyperdrive + `pg`/node-postgres. Per-request client from `env.HYPERDRIVE.connectionString` + `ctx.waitUntil(client.end())`. Use PostgreSQL `UUID`, `JSONB`, and `TIMESTAMPTZ` types.
